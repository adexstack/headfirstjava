...
public class QuizCardBuilder {
    ...
    private void saveFile(File file) {
        ...
        */
        // Using optimised try-with-resources to replace the above old style try-catch-finally code
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(file))) {
            for (QuizCard card : cardList) {
                //writer.write(card.getQuestion() + "/");
                //writer.write(card.getAnswer() + "\n");
                // use writer above to replace all in file but append to keep and write on top
                writer.append(card.getQuestion()).append("/");
                writer.append(card.getAnswer()).append("\n");
            }
        } catch (IOException e) {
            System.out.println("Couldn't write the cardList out: " + e.getMessage());
        }
